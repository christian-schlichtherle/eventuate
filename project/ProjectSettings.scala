...
object ProjectSettings {
  ...
  lazy val exampleSettings: Seq[Setting[_]] = {
    ...
    Seq(
      ...
      generateClasspath := {
        val currentDirectory = new File(sys props "user.dir")
        val classpaths = (fullClasspath in Compile)
          .value
          .map(_.data)
          .map(file => file relativeTo currentDirectory getOrElse file)
          .mkString(":")
        val output =
          s"""|#!/bin/sh
              |
              ...
              |
              |export EXAMPLE_CLASSPATH="$classpaths"
              |""".stripMargin
